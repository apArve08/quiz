...
const C = {
  ...
  p15:        "B8901A",  // bronze - Timeseries Compressor
  heading:    "1B2A4A",
  accent:     "2E5090",
  muted:      "5A6A8A",
  white:      "FFFFFF",
...
const phases = [
  ...
  { num: "V",   title: "Hardware & Memory Optimization",          weeks: "Weeks 35–43", desc: "Low-level hardware engineering — GPU memory tiling, PCIe bus profiling, heterogeneous CPU/VRAM model offloading, and byte-level stream compression." },
];
...
const projects = [
  ...
  // ── PROJECT 15 ─────────────────────────────────────────────────────────────
  {
    id: "P15", color: C.p15, phase: "V", weeks: "Weeks 41–43",
    title: "High-Throughput Timeseries Chunk Compressor",
    subtitle: "Zstd Dictionary Training · Error-Bounded Lossy Quantization · Streaming Serialization",
    bottleneck: "Resolves packet-header overhead waste and per-message dictionary rebuild costs that dominate the storage footprint of repetitive timeseries telemetry. Generic compression algorithms treat each small message as an independent stream, repeatedly paying dictionary reconstruction cost and failing to exploit schema-level redundancy across messages. This project builds a custom Zstd dictionary from a representative training corpus of telemetry samples, then applies error-bounded lossy pre-quantization to floating-point metrics before compression — dramatically reducing the byte count of high-frequency timeseries payloads while preserving bounded numerical fidelity.",
    overview: "The Timeseries Chunk Compressor targets a specific class of high-volume data: repeating-schema telemetry (sensor readings, transaction logs, application metrics) where small individual messages are emitted at high frequency. Three techniques combine for compounding gains: (1) Zstd dictionary training produces a small binary file that pre-loads the compressor with the recurring schema tokens, eliminating per-message dictionary overhead. (2) Error-bounded lossy quantization rounds floating-point values to a configurable epsilon (e.g., 0.01% relative error) before they enter the compressor, reducing the entropy of the input and improving compression ratio. (3) Streaming serialization feeds compressed chunks into in-memory byte buffers that can be transmitted over the network as a single batch, amortising per-message framing overhead.",
    objectives: [
      "Generate a custom Zstd dictionary from a representative training corpus of repeating-schema telemetry samples.",
      "Implement an error-bounded lossy quantizer for floating-point timeseries values with configurable relative error epsilon.",
      "Build a streaming chunk serializer that batches small messages into a single compressed buffer.",
      "Benchmark compression ratio, encode/decode latency, and CPU overhead across lossy and lossless configurations.",
      "Validate lossless fidelity: decompressed output matches input within the configured epsilon bound for every value.",
    ],
    stack: {
      "Language": "Python 3.11 (zstandard library), C++ (libzstd FFI for hot-path compression)",
      "Compression": "zstandard (Python binding for libzstd 1.5+), supports trainDict and compressUsingDict APIs",
      "Data Format": "JSON input corpus, MessagePack or CBOR for compact serialization (optional)",
      "Visualisation": "Matplotlib (compression ratio curves, latency vs. epsilon tradeoff)",
      "Build": "libzstd-dev system package, pip install zstandard",
    },
    architecture: [
      "Dictionary Training Phase: A corpus of 1,000–10,000 representative timeseries samples is collected from real or synthetic telemetry (sensor_id, timestamp, metric_value, unit). The zstandard.train_dict() function analyses byte-level patterns and produces a small binary dictionary file (typically 8–64KB) that pre-loads the compressor with the recurring schema and value patterns. The dictionary is persisted to disk and loaded at encoder startup.",
      "Error-Bounded Lossy Quantizer: For floating-point metric_value fields, the quantizer applies rounding to a configurable number of significant digits determined by epsilon (e.g., epsilon=0.0001 retains 4 significant digits, achieving ≤ 0.01% relative error). The function q(v) = round(v * 1/eps) * eps quantises each value, and the inverse q^-1 reconstructs it with bounded error. The quantizer is applied in-place to the message dictionary before compression.",
      "Lossless Schema Fields: Non-numeric fields (sensor_id, timestamp, unit) pass through the quantizer unchanged — only float values are quantised. This preserves exact schema fidelity while compressing the high-entropy numeric payload.",
      "Streaming Chunk Serializer: Incoming messages are appended to an in-memory bytearray buffer. A flush trigger fires when (a) buffer size exceeds a configurable threshold (default 4KB) or (b) the time since last flush exceeds a configurable interval (default 100ms). On flush, the buffer is compressed using the pre-trained dictionary and emitted as a single chunk. The receiver decompresses chunks independently and appends messages to a queue.",
      "Decode Path with Bounded Error: The decompressor uses the same dictionary to decode each chunk. After decompression, lossy-quantized float values are dequantised back to float. The decoder asserts |original - decoded| / |original| ≤ epsilon for every value, logging any violations.",
    ],
    weekPlan: [
      { num: 41, title: "Dictionary Training + Baseline", tasks: ["Generate synthetic timeseries corpus: 5,000 JSON samples with fixed schema and varying float values (normal distribution, mean 50.0, std 5.0)", "Train Zstd dictionary using zstandard.train_dict() with corpus samples", "Save dictionary to disk, measure file size (target < 64KB)", "Benchmark baseline: compress 1,000 samples individually without dictionary, log total bytes", "Benchmark with dictionary: compress same 1,000 samples with dictionary, log total bytes and ratio", "Plot ratio improvement vs. sample size: identify crossover where dictionary pays off"], deliverable: "Trained Zstd dictionary file with measured compression ratio improvement over generic compression" },
      { num: 42, title: "Error-Bounded Lossy Quantization", tasks: ["Implement quantize(v, eps) and dequantize(v, eps) functions with relative error bound assertion", "Apply quantizer with eps=0.0001 (4 significant digits) to corpus before compression", "Benchmark: quantize-then-compress vs. compress-only — measure ratio gain", "Validate: decompress 1,000 samples, assert |original - decoded| / |original| ≤ 0.0001 for every value", "Sweep eps values: 0.001, 0.0001, 0.00001 — plot ratio vs. precision tradeoff curve", "Implement adaptive quantizer: per-field epsilon configuration for fields with different precision requirements"], deliverable: "Lossy quantizer with validated bounded error, epsilon sweep results, and per-field adaptive configuration" },
      { num: 43, title: "Streaming Serializer + Final Dashboard", tasks: ["Implement streaming buffer: append messages, flush on size or time threshold", "Build chunk emitter: compressed chunk format includes magic bytes, dictionary ID, decompressed size for integrity", "Benchmark end-to-end: ingest 100,000 messages, measure total bytes, encode/decode latency", "Plot 4-panel dashboard: ratio vs. sample count, latency vs. epsilon, throughput vs. buffer size, fidelity vs. epsilon", "Write analysis: when does dictionary + lossy outperform generic lossless? (sample count, schema repetition)", "Package as Python module: Compressor class with train(), compress_stream(), decompress_stream() methods"], deliverable: "Streaming serializer with 4-panel dashboard, end-to-end benchmark, and importable Python module" },
    ],
    learnings: [
      "Zstd dictionary training mechanics: how the dictionary analyser identifies repeating substrings across samples and encodes them as a shared prefix table.",
      "Error-bounded lossy compression theory: the precision-vs-ratio tradeoff and why bounded error is acceptable for telemetry use cases (sensor noise exceeds quantization error).",
      "Streaming serialization patterns: framing compressed chunks with magic bytes and size headers so receivers can decompress independently without out-of-band coordination.",
      "Compression ratio dynamics: small messages suffer from header overhead; batching into chunks amortises framing cost — the same insight that motivates HTTP/2 header compression.",
      "Byte-level engineering: writing hot-path compression in C++ (via libzstd FFI) versus Python — the 10× latency difference is invisible at small message counts but dominates at millions-per-second throughput.",
    ],
    metrics: [
      ["Compression Ratio",       "≥ 5× over raw JSON, ≥ 2× over generic zstd", "Bytes in / bytes out measurement",  "Week 41"],
      ["Quantizer Fidelity",      "0 violations across 1K samples",           "Per-value error assertion",          "Week 42"],
      ["Encode Latency",          "< 50µs per message (batch amortised)",     "Python time benchmark",               "Week 43"],
      ["Throughput",              "≥ 100K messages/sec sustained",            "End-to-end stream benchmark",         "Week 43"],
    ],
    warning: "Zstd dictionary training requires the input samples to be representative of the production data distribution. If the training corpus contains only 3 sensor types but production has 50, the dictionary will underperform because the new schema patterns are absent. Always retrain the dictionary when the message schema changes — a stale dictionary is worse than no dictionary (generic compression falls back gracefully; a bad dictionary actively hurts the ratio).",
    tip: "This project directly extends your MES background. Production floor telemetry — machine vibration, temperature, cycle counts — is exactly the repeating-schema timeseries data this compressor targets. The error-bounded lossy quantizer is conceptually identical to the rounding tolerance in your SCADA dashboards. Build a sample corpus from your actual process data to make the benchmark results personally meaningful.",
  },
];
...
allChildren.push(
  ...
  new Table({
    ...
    rows: [new TableRow({ children: [
      cell([para("15 Projects", { bold: true, color: C.white, size: 32, align: AlignmentType.CENTER, after: 20 }), para("Across 5 Phases", { color: "D0DCF0", size: 20, align: AlignmentType.CENTER, after: 0 })], { fill: C.navy, width: CONTENT_W/3 }),
      cell([para("43 Weeks", { bold: true, color: C.white, size: 32, align: AlignmentType.CENTER, after: 20 }), para("~10 Months", { color: "D0DCF0", size: 20, align: AlignmentType.CENTER, after: 0 })], { fill: C.darkBlue, width: CONTENT_W/3 }),
      cell([para("Full Stack to", { bold: true, color: C.white, size: 26, align: AlignmentType.CENTER, after: 20 }), para("Byte Level", { color: "D0DCF0", size: 20, align: AlignmentType.CENTER, after: 0 })], { fill: C.midBlue, width: CONTENT_W/3 }),
    ]})]
  }),
  spacer(480),
  new Paragraph({
...
allChildren.push(
  ...
  para("This document covers all 15 projects across 5 engineering phases. Each section contains: project overview, system bottleneck analysis, architectural design, week-by-week task breakdown, learning outcomes, success metrics, and critical implementation notes.", { size: 21, color: C.darkGray, after: 240 }),
  new Table({
    width: { size: CONTENT_W, type: WidthType.DXA },
...
allChildren.push(
  ...
  para("The following environment configuration is mandatory before beginning any project. All 15 projects share a common infrastructure base. Establishing this foundation correctly in Week 0 prevents environment-related interruptions throughout the programme.", { size: 21, color: C.darkGray, after: 200 }),
  twoColTable(
    "Component", "Setup Requirement",
    ["Container Orchestration", "Local Model Deployment (P1–P5)", "Phase 2 Agent Models", "Monorepo Structure", "Financial API", "Load Testing", "Neural Computing", "Engineering Log", "Compression Library", "Byte-Level Dev Tools"],
    ["Docker Desktop — manages PostgreSQL, Redis, Ollama containers", "Ollama: pull llama3.2:3b, qwen2.5-coder:7b, nomic-embed-text", "DeepSeek-r1 (Architect), Qwen2.5-coder (Developer), Llama3.1 (QA)", "Single GitHub monorepo with one folder per project", "Register Finnhub API key (free tier, ~60 calls/min)", "Install k6 v0.51+ for high-concurrency load testing", "Install PyTorch 2.2 and NumPy 1.26 for tensor operations (P6–P14)", "Obsidian or Notion: weekly log — Built / Broken / Resolved", "Install libzstd-dev (apt) and zstandard (pip) for P15 dictionary compression", "gdb, ltrace, strace, xxd, hexdump — for inspecting byte-level stream outputs"],
    C.white, C.navy
  ),
  spacer(160),
  noteBox("CRITICAL", "Create a /mocks fixture directory in every project folder on Day 1. All external API responses (Finnhub, Ollama) must be mirrored to local JSON files on first invocation. This single practice prevents external rate limits and network availability from blocking development momentum across the entire 43-week timeline. For P15, additionally create a /corpus directory containing 5,000+ representative timeseries samples for dictionary training.", C.warning, C.warnBg),
  spacer(240),
  new Paragraph({ children: [new PageBreak()] })
);
...
allChildren.push(
  ...
  para("Upon completion of all 15 projects across 43 weeks, the following engineering competencies are demonstrably acquired through working, tested, and deployed software systems.", { size: 21, color: C.darkGray, after: 200 }),
  new Table({
    width: { size: CONTENT_W, type: WidthType.DXA },
    ...
    rows: [
      ...
      ...[
        ...
        ["Byte-Level Systems",  "Zstd dictionary training, error-bounded quantization, streaming serialization", "P15 compression ratio dashboard, fidelity validation report"],
  ...
  spacer(200),
  para("Every competency above is backed by a concrete, testable software artifact — not theoretical study notes. The portfolio produced by this programme represents demonstrable engineering capability across the full AI systems stack from infrastructure to hardware to byte-level byte streams.", { size: 21, color: C.muted, italic: true, align: AlignmentType.CENTER, before: 120, after: 0 })
);
...
Packer.toBuffer(doc).then(buffer => {
  fs.writeFileSync("/Users/ap/quiz/AI_Engineering_Roadmap1.docx", buffer);
  console.log("Done. Saved to /Users/ap/quiz/AI_Engineering_Roadmap1.docx");
});
